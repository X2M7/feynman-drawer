...
## 功能亮点
- 基础元素：点 / 直线 / 曲线 / 椭圆 / 标签
- 线型与样式：实线、虚线、点线、波浪线、胶子线，支持箭头与线宽/颜色
- 交互：框选、复制、组合/取消组合；选中曲线可拖动控制点；标签可绑定到线的起点/中点/终点并设置偏移
- 视图：平移、缩放、可调网格与可开关吸附；画板和 TikZ 面板可拖动分隔线调宽；支持触控双指缩放
- 代码：TikZ 子集编辑与应用（Ctrl/⌘ + Enter），一键复制/导出
- 导出：TikZ 代码、PNG 图片与 SVG 矢量图
...
- 框选：选择工具下在空白处拖动；`Ctrl/⌘ + D` 复制，`Ctrl/⌘ + G` 组合，`Ctrl/⌘ + Shift + G` 取消组合
- 网格：可独立开关网格显示和吸附，并用“格距”输入框调整间距
- 面板：拖动画板与 TikZ 代码之间的分隔线调整比例，双击分隔线恢复默认
- 曲线控制点：选中曲线后拖动白色控制点调整弯曲；虚线连接其与两端点，调整时两个端点保持固定
